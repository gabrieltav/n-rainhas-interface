/**
 *  Mecanismo baseado em algoritmo genético para resolver o problema de n-rainhas
 *  Desenvolvedores : Carlos Eduardo, Gabriel Tavares, Lucas Weslen e Marcos Vinicius
 *  Uso: crie o objeto desta classe e chame o gerador object.run() para obter a solução passo a passo
 *  Dependências : ./genetic-tools.js
 */
...
class NQueensGeneticEngine {
  ...
  *run() {
    let generation = Array(this.config.populationSize)
      .fill(0)
      .map(() => this.generateDNA());
    let bestDNA;
    ...
    do {
      ...
      const selectedIndexes = Array(generation.length)
        .fill(0)
        .map(() => GeneticAlgorithm.selection.RouletteWheel(fitnesses));
      const pairedIndexes = GeneticAlgorithm.pairing.Fitness(
        selectedIndexes,
        (index) => fitnesses[index]
      );
      ...
      for (let pairedIndex in pairedIndexes) {
        ...
        const [parent1, parent2] = pairedIndexes[pairedIndex].map(
          (i) => generation[i]
        );
      ...
      // mutation
      for (let i = 0; i < Math.floor(generation.length * 0.2); i++) {
